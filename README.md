...
| משתנה | הסבר |
|---|---|
| `SITE_URL` | כתובת האתר בפועל, **כולל** הסכימה `https://` בהתחלה ו**בלי** `/` בסוף (למשל `https://example.com`, לא `example.com`). חשוב לעדכן כשעוברים מ-localhost לדומיין האמיתי. |
...
האתר שולח שני סוגי מייל, שניהם דרך אותה הגדרת SMTP (ראו למטה):

- **קבלת פנים בהרשמה** - מיד לאחר הרשמה מוצלחת, המשתמש מקבל מייל "ברוכים
  הבאים" עם שמו, שם הקורס וקישור לכניסה. **בכוונה אין במייל הזה את
  הסיסמה** - שליחת סיסמה בטקסט גלוי במייל היא פרקטיקת אבטחה גרועה (מייל
  לרוב לא מוצפן, נשמר/מועבר לאורך זמן, וכל מי שיגיע לתיבת הדואר מקבל
  גישה לחשבון), וגם סותרת את מדיניות הפרטיות של האתר עצמו שמבטיחה שהסיסמה
  לא נגישה לעיון גם עבורנו. המשתמש כבר יודע את הסיסמה שלו - הוא הרגע בחר
  אותה בטופס ההרשמה.
- **איפוס סיסמה** - יש קישור "שכחתם סיסמה?" בעמוד ההתחברות. כשמשתמש מבקש
  איפוס, האתר שולח מייל עם קישור חד-פעמי שתקף לשעה. הקישור מאפשר לבחור
  סיסמה חדשה ומתחבר אוטומטית לאחר מכן (וגם מבטל סשנים פתוחים אחרים של
  אותו משתמש, כהגנה).

**בלי הגדרת SMTP** - האתר עדיין "עובד", אבל במקום לשלוח מייל קבלת פנים
המערכת רק רושמת ללוג שהוא לא נשלח, ובמקום מייל איפוס סיסמה, קישור
האיפוס עצמו יודפס ללוג השרת (`console.warn`) עם הכתובת שביקשה אותו. זה
שימושי לבדיקות, אבל לאתר חי כדאי להגדיר שליחת מייל אמיתית.
...
- **Vimeo**
- **Cloudflare Stream** - אפשר להדביק כל אחד מהקישורים הבאים והאתר ידע
  להטמיע אותם: קישור ה-"Watch" (`watch.cloudflarestream.com/<uid>`), קוד
  ההטמעה (`customer-<code>.cloudflarestream.com/<uid>/iframe`), או סתם
  ה-UID של הסרטון בתור נתיב (`.../<uid>` בלי `/iframe` בסוף - האתר משלים
  את זה אוטומטית). אם יוצרים **Signed URL** (טוקן זמני לוידאו פרטי) -
  אפשר להדביק את הקישור המלא כולל `?token=...` והוא יישמר.
...
שמגבילה הטמעה רק לדומיין שלכם, ו-**Cloudflare Stream** מאפשר גם
"domain-level restriction" וגם Signed URLs עם תוקף מוגבל - השילוב הכי הדוק
מבין האפשרויות כאן, אם מוכנים להקים חשבון Cloudflare Stream (בתשלום לפי
דקות צפייה/אחסון).
...
- קישור איפוס הסיסמה הוא טוקן חד-פעמי שתקף לשעה; במסד הנתונים נשמר רק
  ...
  כתובות רשומות באתר.
- יש הגבלת קצב (rate limiting) בזיכרון על הרשמה/התחברות/כניסת ניהול/איפוס
  סיסמה (`lib/rate-limit.js`), לפי כתובת IP. היא מתאימה לתהליך שרת יחיד
  בלבד - אם בעתיד תריצו כמה instances מאחורי load balancer, יש להחליף
  אותה במימוש משותף (Redis וכו'), אחרת כל instance סופר בנפרד.
