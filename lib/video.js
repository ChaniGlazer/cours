...
export function parseVideoEmbed(url) {
  ...
  try {
    ...
    if (host === "vimeo.com") {
      ...
      const id = segments[0];
      // קישורים פרטיים/לא-רשומים של Vimeo כוללים hash שני - כנתיב שני
      // (vimeo.com/<id>/<hash>) או כפרמטר שאילתה (?h=<hash>). בלי ה-hash
      // הזה, הטמעת וידאו פרטי תיכשל עם "This video is private".
