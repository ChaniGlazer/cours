...
export function createUser({ name, email, passwordHash, termsAcceptedAt }) {
  const id = crypto.randomUUID();
  db.prepare(
    "INSERT INTO users (id, name, email, password_hash, paid, terms_accepted_at, created_at) VALUES (?, ?, ?, ?, 0, ?, ?)"
  ).run(id, name, email.toLowerCase().trim(), passwordHash, termsAcceptedAt || null, nowIso());
  return id;
}
